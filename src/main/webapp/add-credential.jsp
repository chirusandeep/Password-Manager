	...
	<div class="container d-flex justify-content-center">
		<main class="w-50">
			<form method="POST" action="credentials">
				<h1 class="h3 mt-3 mb-3 text-center">Add Credential Form</h1>
				<c:if test="${not empty message}">
					<div id="message" class="alert alert-success text-center">${message}</div>
				</c:if>

				<c:if test="${not empty errorMessage}">
					<div id="message" class="alert alert-danger text-center">${errorMessage}</div>
				</c:if>
				...
				<br/>
				<button id="addcred" class="w-100 btn btn-lg btn-primary"
					type="submit">Add Credential</button>
			</form>
